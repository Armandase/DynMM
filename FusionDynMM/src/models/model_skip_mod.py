...
class SkipESANet(nn.Module):
    def __init__(
        ...
        if fuse_depth_in_rgb_encoder == "SE-add":
            ...
            self.se_layer1 = SqueezeAndExciteFusionAdd(
                self.encoder_rgb.down_4_channels_out, activation=self.activation
            )
            self.se_layer2 = SqueezeAndExciteFusionAdd(
                self.encoder_rgb.down_8_channels_out, activation=self.activation
            )
            self.se_layer3 = SqueezeAndExciteFusionAdd(
                self.encoder_rgb.down_16_channels_out, activation=self.activation
            )
            self.se_layer4 = SqueezeAndExciteFusionAdd(
                self.encoder_rgb.down_32_channels_out, activation=self.activation
            )

        # gating network & gumbel softmax
        self.temp = temp
        self.gate_layer0 = SqueezeAndExciteReweigh(
            self.temp, 64, activation=self.activation
        )
        self.gate_layer1 = SqueezeAndExciteReweigh(
            self.temp, self.encoder_rgb.down_4_channels_out, activation=self.activation
        )
        self.gate_layer2 = SqueezeAndExciteReweigh(
            self.temp, self.encoder_rgb.down_8_channels_out, activation=self.activation
        )
        self.gate_layer3 = SqueezeAndExciteReweigh(
            self.temp, self.encoder_rgb.down_16_channels_out, activation=self.activation
        )
    ...
    def forward(self, rgb, depth, test=False):
        ...
        fuse = rgb + depth
        weight0 = self.gate_layer0(
            rgb, depth, hard=self.hard_gate, random=self.random_policy, test=test
        )
        if self.save_weight_info:
            self.weight_list[0] = torch.cat(
                (self.weight_list[0], weight0[:, :, 0, 0].cpu())
            )
        ...
        if self.save_weight_info:
            self.weight_list[1] = torch.cat(
                (self.weight_list[1], weight1[:, :, 0, 0].cpu())
            )
        ...
        if self.save_weight_info:
            self.weight_list[2] = torch.cat(
                (self.weight_list[2], weight2[:, :, 0, 0].cpu())
            )
        ...
        if self.save_weight_info:
            self.weight_list[3] = torch.cat(
                (self.weight_list[3], weight3[:, :, 0, 0].cpu())
            )
