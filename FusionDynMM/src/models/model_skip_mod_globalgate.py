...
from src.models.rgb_depth_fusion import (
    SqueezeAndExciteFusionAdd,
    SqueezeAndExciteReweigh,
)
...
from src.models.fusion_methods import select_fusion_method
...
class SkipGateESANet(nn.Module):
    def __init__(
        ...
        nb_channel_rgb=3,
        nb_channel_nir=1,
    ):  # 0: rgb only, 1: rgb+d, others: dynamic
        ...
        self.block_rule = (
            block_rule if block_rule else [1, 1, 1, 1]
        )  # [0, 0, 0, 0] rgb;  [1, 1, 0, 0] skip last 2 fusion components
        ...
        if encoder_rgb == "resnet50" or encoder_depth == "resnet50":
            warnings.warn(
                "Parameter encoder_block is ignored for ResNet50. "
        ...
        self.fusion_layer0 = select_fusion_method(fuse_depth_in_rgb_encoder, channel=64)
        self.fusion_layer1 = select_fusion_method(
            fuse_depth_in_rgb_encoder, channel=self.encoder_rgb.down_4_channels_out
        )
        self.fusion_layer2 = select_fusion_method(
            fuse_depth_in_rgb_encoder, channel=self.encoder_rgb.down_8_channels_out
        )
        self.fusion_layer3 = select_fusion_method(
            fuse_depth_in_rgb_encoder, channel=self.encoder_rgb.down_16_channels_out
        )
        self.fusion_layer4 = select_fusion_method(
            fuse_depth_in_rgb_encoder, channel=self.encoder_rgb.down_32_channels_out
        )
        ...
        if "learned-3x3" in upsampling:
            warnings.warn(
                "for the context module the learned upsampling is "
        ...
        if encoder_rgb == "resnet34":
            self.flop = torch.Tensor([0, 3.27, 7.27, 13.15, 16.02]).to(device)
            self.depth_enc_flop = torch.Tensor(
                [0.2506752, 3.1113216, 6.9470208, 12.66432, 15.538944]
            ).to(device)
            self.total_flop = torch.Tensor(
                [22.37101509, 25.23166149, 29.06736069, 34.78465989, 37.65928389]
            ).to(device)
        else:
            self.depth_enc_flop = torch.Tensor(
                [0.2506752, 4.39420573, 10.72382115, 19.71582947, 24.679084]
            ).to(device)
            self.total_flop = torch.Tensor(
                [32.5854654, 36.728995928, 43.058611352, 52.050619672, 57.0138742]
            ).to(device)
    ...
    def end_weight(self, print_each=False, print_flop=False):
        ...
        for i in range(5):
            ...
            if print_flop:
                cnt_list[i] = tmp_cnt
        if print_flop:
            cnt_list = torch.from_numpy(cnt_list / cnt_list.sum()).to(device)
    ...
    def forward(self, rgb, depth, test=False, return_weight=False):
        ...
        branch0 = rgb
        branch1 = self.fusion_layer1(rgb, depth)
        # print(weight.shape)
        ...
        branch0 = rgb
        branch1 = self.fusion_layer2(rgb, depth)
        ...
        branch0 = rgb
        branch1 = self.fusion_layer3(rgb, depth)
        ...
        branch0 = rgb
        branch1 = self.fusion_layer4(rgb, depth)
    ...
    #     else:
    #         depth = self.encoder_depth.forward_layer1(depth)
    #         fuse = self.fusion_layer1(rgb, depth)
    #     skip1 = self.skip_layer1(fuse)
    ...
    #     else:
    #         depth = self.encoder_depth.forward_layer2(depth)
    #         fuse = self.fusion_layer2(rgb, depth)
    #     skip2 = self.skip_layer2(fuse)
    ...
    #     else:
    #         depth = self.encoder_depth.forward_layer3(depth)
    #         fuse = self.fusion_layer3(rgb, depth)
    #     skip3 = self.skip_layer3(fuse)
    ...
    #     else:
    #         depth = self.encoder_depth.forward_layer4(depth)
    #         fuse = self.fusion_layer4(rgb, depth)
...
def see_gate_output():
    ...
    rgb = torch.randn(
        6, 64, 120, 160
    )  # (64, 120, 160), (64, 120, 160), (128, 60, 80), (256, 30, 40)
